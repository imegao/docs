...
> pip提示版本问题 pip install --upgrade pip升级
...
```
出现 timeout 报错是因为网络问题下载失败 ，重新执行命令。
## 滚动升级 TiDB 集群组件
...
> 提示swap问题使用 swapoff -a 命令关闭
> 在滚动升级的最后阶段 tidb会重启断开1s左右，线上操作的话，请做好相关风险控制。
## 滚动升级 TiDB 监控组件

监控组建无特殊需求不要升级。如果需要升级，之前未用tidb,自己部署过 prometheus alertmanager node_exporter grafana 该命令会无法关闭端口出现错误，请执行kill相关进程。
